...
class ReceivablePayableReport(object):
	...
	def update_voucher_balance(self, gle):
		...
		gle_balance = self.get_gle_balance_in_account_currency(gle)
...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def get_columns(self):
		...
		for i in currencies:
			self.add_column(label=i, fieldname=frappe.scrub(i), fieldtype="Currency", width=140)


def get_currencies():
	return frappe.db.sql_list("select name from `tabCurrency`  where enabled = 1 order by name asc")
